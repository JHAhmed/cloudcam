...
export async function load({ request, url, locals }) {  // Assuming you pass locals from hooks
    const isAuthenticated = await kindeAuthClient.isAuthenticated(request);
    const defaultSettings = { theme: 'dark', imagePersistence: 'on' };

    if (isAuthenticated) {
        const user = await kindeAuthClient.getUser(request);
        let settings = locals.cachedSettings;  // Check if already cached in locals

        if (!settings) {
            settings = await appwrite.tablesDB.getRow("users", "settings", user.id);
            locals.cachedSettings = settings || defaultSettings;  // Cache for the request
        }
